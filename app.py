...
def handle_message(event):
    ...
    elif message_text in ["我想要訂購", "add"]:
    ...
    # 當user要訂購時就會執行這段程式
        ...
        # 資料庫搜尋是否有這個產品名稱
        product = (
            db_session.query(Products).filter(Products.name.ilike(product_name)).first()
        )
        # 如果有這個產品名稱就會加入
        if product:
            ...
            message = TemplateSendMessage(
                alt_text="anything else?", template=confirm_template
            )
    ...
    if action == "checkout":  # 如果action裡面的值是checkout的話才會執行結帳的動作
        ...
        for product_name, num in cart.bucket().items():  # 透過迴圈把項目轉成訂單項目物件
            ...
            product = (
                ...
                .filter(Products.name.ilike(product_name))
                .first()
            )
            # 接著產生訂單項目的物件
            item = Items(
        ...
        message = TemplateSendMessage(
            ...
            template=ButtonsTemplate(
                ...
                actions=[
                    URIAction(label="Pay NT${}".format(order.amount), uri=pay_web_url)
                ],
            ),
        )
...
def confirm():
    ...
    order = (
        db_session.query(Orders).filter(Orders.transaction_id == transaction_id).first()
    )
